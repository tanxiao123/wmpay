...
public class WmAuthGroupService {
	...
	@Autowired WmAuthRuleDAO wmAuthRuleDAO;
	...
	public String getGroupByAuthGroupId(Integer wmAuthGroupId) {
		...
		// 获取管理员所拥有的菜单权限
		WmAuthGroup adminGroup = wmAuthGroupDAO.selectById(wmAuthGroupId);
		String adminRule = adminGroup.getRules();
		String[] adminRules = adminRule.split("\\|");
		Set<String> set = new HashSet<String>(Arrays.asList(adminRules));
		
		// 遍历父节点菜单
		for(int i = 0; i < parentMenus.size(); i++) {
			String parentMenuCheck = set.contains(String.valueOf(menus.get(i).getWmAuthRuleId() )) ? "checked" : "";
			html.append("<dl class=\"Hui-admin-permission-list\">");
			html.append("<dt>");
			html.append("<label><input type=\"checkbox\" "+parentMenuCheck+"  name=\"user-Character-0\" id=\"user-Character-"+i+"\">"+parentMenus.get(i).getTitle()+"</label>");
			html.append("</dt>");
			...
			for(int j = 0; j < menus.size(); j++) {
				if (menus.get(j).getParentId().equals(parentMenus.get(i).getWmAuthRuleId() ) ) {
					...
					menuBuffer.append("<input type=\"checkbox\" "+menuCheck+"  name=\"user-Character-"+i+"-"+j+"\" id=\"user-Character-"+i+"-"+j+"\">"+menus.get(j).getTitle());
					menuBuffer.append("</label>");
					// 查询子级菜单下的功能权限
					for(int k = 0; k<ppMenus.size(); k++) {
						if (ppMenus.get(k).getParentId().equals(menus.get(j).getWmAuthRuleId() ) ) {
							String ppMenuCheck = set.contains(String.valueOf(menus.get(j).getWmAuthRuleId() ) ) ? "checked" : "";
							ppMenuStr.append("<label>");
							ppMenuStr.append("<input type=\"checkbox\" "+ppMenuCheck+"  name=\"user-Character-"+i+"-"+j+"-"+k+"\" id=\"user-Character-"+i+"-"+j+"-"+k+"\">"+ppMenus.get(k).getTitle() );
							ppMenuStr.append("</label>");
						}
					}
					menuBuffer.append("</dt>");
